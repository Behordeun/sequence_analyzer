...
# Reset session state per page
if st.session_state.get("last_page") != "DNA_Analysis":
...
uploaded_file = st.file_uploader("Upload DNA Sequence File", type=["fasta", "txt", "rtf"])
if uploaded_file:
...
if st.session_state["sequences"] and st.button("Analyze"):
    st.session_state["sequence_df"] = analyze_sequences(st.session_state["sequences"], is_rna=False)
